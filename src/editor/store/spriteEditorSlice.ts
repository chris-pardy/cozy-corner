...
export type Tool = "pencil" | "brush" | "fill" | "eraser" | "move";
...
export interface SpriteEditorState {
  ...
  // Version counter (replaces dirty)
  version: number;
}
...
export const spriteEditorSlice = createSlice({
  ...
  reducers: {
    ...
    setCanvasW(state, action: PayloadAction<number>) {
      ...
      state.tfX = 0;
      state.tfY = 0;
      state.tfW = action.payload;
      state.tfH = state.canvasH;
    },
    setCanvasH(state, action: PayloadAction<number>) {
      state.canvasH = action.payload;
      state.tfX = 0;
      state.tfY = 0;
      state.tfW = state.canvasW;
      state.tfH = action.payload;
    },
    ...
    setActiveLayerId(state, action: PayloadAction<number>) {
    ...
    setLayerTint(
      state,
      action: PayloadAction<{ idx: number; tint: string | null }>,
    ) {
    ...
    // Frames
    addFrame(state) {
      state.currentFrame = state.frameCount;
      state.frameCount += 1;
    },
    ...
    setPreviewFps(state, action: PayloadAction<number>) {
      state.previewFps = action.payload;
    },
    toggleOnionPrev(state) {
      state.onionPrev = !state.onionPrev;
    },
    toggleOnionNext(state) {
      state.onionNext = !state.onionNext;
    },
    toggleShowBg(state) {
    ...
    // Transform
    setTransformAllFrames(state) {
      state.transformAllFrames = !state.transformAllFrames;
    },
    setRotAngle(state, action: PayloadAction<number>) {
      state.rotAngle = action.payload;
    },
    setTransformRect(
      state,
      action: PayloadAction<{ x: number; y: number; w: number; h: number }>,
    ) {
      state.tfX = action.payload.x;
      state.tfY = action.payload.y;
      state.tfW = action.payload.w;
      state.tfH = action.payload.h;
    },
    ...
    restoreState(_state, action: PayloadAction<SpriteEditorState>) {
