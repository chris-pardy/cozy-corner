...
import type { AvatarDispatch } from '../redux/store';
import {
  selectName,
  selectDescription,
  selectTags,
...
export function AvatarEditorApp() {
  ...
  return <AvatarEditorInner did={did!} onSignOut={signOut} />;
}

function AvatarEditorInner({ did, onSignOut }: { did: string; onSignOut: () => void }) {
  const dispatch = useDispatch<AvatarDispatch>();
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <aside className="flex w-72 shrink-0 flex-col gap-1.5 overflow-y-auto border-l border-surface-border bg-surface p-1.5">
          <MetadataPanel
            ...
            descriptionPlaceholder="A cozy character..."
            tagsPlaceholder="character, pixel-art, ..."
          />
          <StatePropertyEditor
